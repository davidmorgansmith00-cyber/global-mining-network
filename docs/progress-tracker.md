...
| Invalid session binding transport counter coverage | Done | Added integration test asserting rejected invalid query and invalid header session bindings increment `operation_intent_transport_requests_total.query` and `.header` exactly once in JSON and expose exact `1` counts in plaintext metrics |
| Strict-mode query rejection plaintext counter exactness coverage | Done | Extended integration test to assert strict header mode query rejections expose exact `gmn_operation_intent_transport_requests_total{mode="query_rejected_strict"} 2` in plaintext metrics |
| Strict-mode dual transport acceptance metrics coverage | Done | Added integration test asserting strict header mode accepts dual query+header bindings for start/stop, increments `operation_intent_transport_requests_total.dual_match` exactly twice, and keeps `.query_rejected_strict` at zero |
...
| Newline header-whitespace transport counter coverage | Done | Added integration test asserting newline header session binding rejection increments `operation_intent_transport_requests_total.header` exactly once and exposes exact plaintext metric count |
| Carriage-return header-whitespace transport counter coverage | Done | Added integration test asserting carriage-return header session binding rejection increments `operation_intent_transport_requests_total.header` exactly once and exposes exact plaintext metric count |
| CRLF header-whitespace transport counter coverage | Done | Added integration test asserting CRLF header session binding rejection increments `operation_intent_transport_requests_total.header` exactly once and exposes exact plaintext metric count |
...
| Operation intent mismatch counter exactness coverage | Done | Added integration test asserting mismatched query/header session bindings for start/stop increment `operation_intent_transport_requests_total.mismatch` exactly twice in JSON and plaintext metrics |
...
| Operation intent expired-session query counter exactness coverage | Done | Added integration test asserting expired query session bindings for start/stop increment `operation_intent_transport_requests_total.query` exactly twice in JSON and plaintext metrics |
...
| Query-limit lower-bound acceptance coverage | Done | Added integration test asserting status/network-snapshot/rewards/network-events endpoints accept configured minimum query limits (`1`, `1`, `1`, `after_sequence=0&limit=1`) with HTTP 200 |
...
| Network-events default-omission query coverage | Done | Added integration test asserting network-events accepts omitted query params (default cursor/limit) and returns HTTP 200 contract payload |
...
| Operation intent whitespace transport counter exactness coverage | Done | Added integration test asserting whitespace-only query and header session bindings reject with HTTP 401 while incrementing `operation_intent_transport_requests_total.query` and `.header` exactly once in JSON and plaintext metrics |
| Operation intent empty-session counter exactness coverage | Done | Added integration test asserting empty query and header session bindings reject with HTTP 401 while incrementing `operation_intent_transport_requests_total.missing` exactly twice in JSON and plaintext metrics |
...
| Carriage-return recent-limit query coverage | Done | Added integration test asserting status, network snapshot, and reward history endpoints reject carriage-return `recent_limit` query values with HTTP 422 |
| CRLF recent-limit query coverage | Done | Added integration test asserting status, network snapshot, and reward history endpoints reject CRLF `recent_limit` query values with HTTP 422 |
...
| Carriage-return network-events query coverage | Done | Added integration test asserting network-events rejects carriage-return `after_sequence` and `limit` query values with HTTP 422 |
| CRLF network-events query coverage | Done | Added integration test asserting network-events rejects CRLF `after_sequence` and `limit` query values with HTTP 422 |
...
| Newline-whitespace cleanup query coverage | Done | Added integration test asserting cleanup rejects newline-whitespace `event_retention_seconds`, `checkpoint_retention_seconds`, and `max_network_events` query values with HTTP 422 |
| Carriage-return cleanup query coverage | Done | Added integration test asserting cleanup rejects carriage-return `event_retention_seconds`, `checkpoint_retention_seconds`, and `max_network_events` query values with HTTP 422 |
| CRLF cleanup query coverage | Done | Added integration test asserting cleanup rejects CRLF `event_retention_seconds`, `checkpoint_retention_seconds`, and `max_network_events` query values with HTTP 422 |
