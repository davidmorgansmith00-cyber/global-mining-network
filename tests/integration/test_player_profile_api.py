...
class PlayerProfileApiIntegrationTests(unittest.TestCase):
    ...
    def test_profile_endpoint_returns_v11_effective_hashrate_contract(self) -> None:
        ...
        try:
            with TestClient(app) as client:
                ...
                self.assertEqual(payload["schema_version"], "player.profile.v1.1")
    ...
    def test_hardware_change_triggers_profile_hashrate_recalculation(self) -> None:
        ...
        try:
            with TestClient(app) as client:
                ...
                updated_profile = PlayerProfileService().assign_hardware_state(
                    ...
                    power_available=45.0,
                    power_capacity=90.0,
                    cooling_efficiency=0.5,
                )
                self.assertEqual(updated_profile.effective_hashrate, 6.0)
                ...
                self.assertEqual(payload["power_available"], 45.0)
                self.assertEqual(payload["power_capacity"], 90.0)
                self.assertEqual(payload["cooling_efficiency"], 0.5)
                self.assertEqual(payload["effective_hashrate"], 6.0)

                with psycopg.connect(self.database_url) as connection:
                    with connection.cursor() as cursor:
                        cursor.execute(
                            """
                            SELECT hardware_id, effective_hashrate_cached, effective_hashrate_updated_at
                            FROM players
                            WHERE player_id = %s
                            """,
                            (UUID(player_id),),
    ...
    def test_profile_openapi_contract_lists_v11_hashrate_fields(self) -> None:
        openapi = app.openapi()
