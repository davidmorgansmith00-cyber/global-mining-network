...
class BlockchainStatusApiTests(unittest.TestCase):
    ...
    def test_status_endpoint_rejects_non_integer_recent_limit(self) -> None:
        with TestClient(app) as client:
            response = client.get("/api/v1/blockchain/status?recent_limit=not_an_int")
    ...
    def test_player_reward_history_endpoint_rejects_non_integer_recent_limit(self) -> None:
        with TestClient(app) as client:
            response = client.get("/api/v1/blockchain/players/player_a/rewards?recent_limit=not_an_int")
    ...
    def test_recent_limit_endpoints_reject_whitespace_query_value(self) -> None:
        with TestClient(app) as client:
            status_response = client.get("/api/v1/blockchain/status?recent_limit=%20%20%20")
            snapshot_response = client.get("/api/v1/blockchain/network-snapshot?recent_limit=%20%20%20")
            rewards_response = client.get("/api/v1/blockchain/players/player_a/rewards?recent_limit=%20%20%20")

        self.assertEqual(status_response.status_code, 422)
        self.assertEqual(snapshot_response.status_code, 422)
        self.assertEqual(rewards_response.status_code, 422)

    def test_recent_limit_endpoints_reject_tab_whitespace_query_value(self) -> None:
        with TestClient(app) as client:
            status_response = client.get("/api/v1/blockchain/status?recent_limit=%09")
            snapshot_response = client.get("/api/v1/blockchain/network-snapshot?recent_limit=%09")
            rewards_response = client.get("/api/v1/blockchain/players/player_a/rewards?recent_limit=%09")
    ...
    def test_recent_limit_endpoints_reject_carriage_return_whitespace_query_value(self) -> None:
        with TestClient(app) as client:
            status_response = client.get("/api/v1/blockchain/status?recent_limit=%0D")
            snapshot_response = client.get("/api/v1/blockchain/network-snapshot?recent_limit=%0D")
            rewards_response = client.get("/api/v1/blockchain/players/player_a/rewards?recent_limit=%0D")

        self.assertEqual(status_response.status_code, 422)
        self.assertEqual(snapshot_response.status_code, 422)
        self.assertEqual(rewards_response.status_code, 422)

    def test_recent_limit_endpoints_reject_crlf_whitespace_query_value(self) -> None:
        with TestClient(app) as client:
            status_response = client.get("/api/v1/blockchain/status?recent_limit=%0D%0A")
            snapshot_response = client.get("/api/v1/blockchain/network-snapshot?recent_limit=%0D%0A")
            rewards_response = client.get("/api/v1/blockchain/players/player_a/rewards?recent_limit=%0D%0A")
    ...
    def test_recent_limit_endpoints_reject_fractional_query_value(self) -> None:
        with TestClient(app) as client:
            ...
            snapshot_response = client.get("/api/v1/blockchain/network-snapshot?recent_limit=1.5")
            rewards_response = client.get("/api/v1/blockchain/players/player_a/rewards?recent_limit=1.5")

        self.assertEqual(status_response.status_code, 422)
        self.assertEqual(snapshot_response.status_code, 422)
        self.assertEqual(rewards_response.status_code, 422)

    def test_recent_limit_endpoints_reject_negative_query_value(self) -> None:
        with TestClient(app) as client:
            status_response = client.get("/api/v1/blockchain/status?recent_limit=-1")
            snapshot_response = client.get("/api/v1/blockchain/network-snapshot?recent_limit=-1")
            rewards_response = client.get("/api/v1/blockchain/players/player_a/rewards?recent_limit=-1")

        self.assertEqual(status_response.status_code, 422)
        self.assertEqual(snapshot_response.status_code, 422)
        self.assertEqual(rewards_response.status_code, 422)

    def test_recent_limit_endpoints_accept_default_when_query_is_omitted(self) -> None:
        with TestClient(app) as client:
            status_response = client.get("/api/v1/blockchain/status")
            snapshot_response = client.get("/api/v1/blockchain/network-snapshot")
            rewards_response = client.get("/api/v1/blockchain/players/player_a/rewards")
    ...
    def test_query_limit_endpoints_accept_configured_lower_bounds(self) -> None:
        with TestClient(app) as client:
            status_response = client.get("/api/v1/blockchain/status?recent_limit=1")
            snapshot_response = client.get("/api/v1/blockchain/network-snapshot?recent_limit=1")
            rewards_response = client.get("/api/v1/blockchain/players/player_a/rewards?recent_limit=1")
            events_response = client.get("/api/v1/blockchain/network-events?after_sequence=0&limit=1")
    ...
    def test_network_events_endpoint_rejects_non_integer_limit(self) -> None:
        with TestClient(app) as client:
            response = client.get("/api/v1/blockchain/network-events?after_sequence=0&limit=not_an_int")

        self.assertEqual(response.status_code, 422)

    def test_network_events_endpoint_rejects_empty_query_values(self) -> None:
        with TestClient(app) as client:
            empty_after_sequence = client.get("/api/v1/blockchain/network-events?after_sequence=&limit=10")
            empty_limit = client.get("/api/v1/blockchain/network-events?after_sequence=0&limit=")
    ...
    def test_network_events_endpoint_rejects_tab_whitespace_query_values(self) -> None:
        with TestClient(app) as client:
            tab_after_sequence = client.get(
                "/api/v1/blockchain/network-events?after_sequence=%09&limit=10"
            )
            tab_limit = client.get(
                "/api/v1/blockchain/network-events?after_sequence=0&limit=%09"
            )
    ...
    def test_network_events_endpoint_rejects_carriage_return_whitespace_query_values(self) -> None:
        with TestClient(app) as client:
            carriage_after_sequence = client.get(
                "/api/v1/blockchain/network-events?after_sequence=%0D&limit=10"
            )
            carriage_limit = client.get(
                "/api/v1/blockchain/network-events?after_sequence=0&limit=%0D"
            )

        self.assertEqual(carriage_after_sequence.status_code, 422)
        self.assertEqual(carriage_limit.status_code, 422)

    def test_network_events_endpoint_rejects_crlf_whitespace_query_values(self) -> None:
        with TestClient(app) as client:
            crlf_after_sequence = client.get(
                "/api/v1/blockchain/network-events?after_sequence=%0D%0A&limit=10"
            )
            crlf_limit = client.get(
                "/api/v1/blockchain/network-events?after_sequence=0&limit=%0D%0A"
            )

        self.assertEqual(crlf_after_sequence.status_code, 422)
        self.assertEqual(crlf_limit.status_code, 422)

    def test_network_events_endpoint_rejects_fractional_query_values(self) -> None:
        with TestClient(app) as client:
            fractional_after_sequence = client.get(
                "/api/v1/blockchain/network-events?after_sequence=1.5&limit=10"
            )
            fractional_limit = client.get(
                "/api/v1/blockchain/network-events?after_sequence=0&limit=10.5"
            )

        self.assertEqual(fractional_after_sequence.status_code, 422)
        self.assertEqual(fractional_limit.status_code, 422)
    ...
    def test_cleanup_endpoint_rejects_non_integer_query_parameters(self) -> None:
        headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
        with TestClient(app) as client:
            event_retention_non_integer = client.post(
                "/api/v1/blockchain/maintenance/cleanup?event_retention_seconds=not_an_int",
                headers=headers,
            )
            checkpoint_retention_non_integer = client.post(
                "/api/v1/blockchain/maintenance/cleanup?checkpoint_retention_seconds=not_an_int",
                headers=headers,
            )
            max_network_events_non_integer = client.post(
                "/api/v1/blockchain/maintenance/cleanup?max_network_events=not_an_int",
                headers=headers,
            )

        self.assertEqual(event_retention_non_integer.status_code, 422)
    ...
    def test_cleanup_endpoint_rejects_whitespace_query_parameters(self) -> None:
        headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
        with TestClient(app) as client:
            event_retention_whitespace = client.post(
                "/api/v1/blockchain/maintenance/cleanup?event_retention_seconds=%20%20%20",
                headers=headers,
            )
            checkpoint_retention_whitespace = client.post(
                "/api/v1/blockchain/maintenance/cleanup?checkpoint_retention_seconds=%20%20%20",
                headers=headers,
            )
            max_network_events_whitespace = client.post(
                "/api/v1/blockchain/maintenance/cleanup?max_network_events=%20%20%20",
                headers=headers,
            )

        self.assertEqual(event_retention_whitespace.status_code, 422)
    ...
    def test_cleanup_endpoint_rejects_newline_whitespace_query_parameters(self) -> None:
        headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
        with TestClient(app) as client:
            event_retention_newline = client.post(
                "/api/v1/blockchain/maintenance/cleanup?event_retention_seconds=%0A",
                headers=headers,
            )
            checkpoint_retention_newline = client.post(
                "/api/v1/blockchain/maintenance/cleanup?checkpoint_retention_seconds=%0A",
                headers=headers,
            )
            max_network_events_newline = client.post(
                "/api/v1/blockchain/maintenance/cleanup?max_network_events=%0A",
                headers=headers,
            )

        self.assertEqual(event_retention_newline.status_code, 422)
        self.assertEqual(checkpoint_retention_newline.status_code, 422)
        self.assertEqual(max_network_events_newline.status_code, 422)

    def test_cleanup_endpoint_rejects_carriage_return_whitespace_query_parameters(self) -> None:
        headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
        with TestClient(app) as client:
            event_retention_carriage = client.post(
                "/api/v1/blockchain/maintenance/cleanup?event_retention_seconds=%0D",
                headers=headers,
            )
            checkpoint_retention_carriage = client.post(
                "/api/v1/blockchain/maintenance/cleanup?checkpoint_retention_seconds=%0D",
                headers=headers,
            )
            max_network_events_carriage = client.post(
                "/api/v1/blockchain/maintenance/cleanup?max_network_events=%0D",
                headers=headers,
            )

        self.assertEqual(event_retention_carriage.status_code, 422)
        self.assertEqual(checkpoint_retention_carriage.status_code, 422)
        self.assertEqual(max_network_events_carriage.status_code, 422)

    def test_cleanup_endpoint_rejects_crlf_whitespace_query_parameters(self) -> None:
        headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
        with TestClient(app) as client:
            event_retention_crlf = client.post(
                "/api/v1/blockchain/maintenance/cleanup?event_retention_seconds=%0D%0A",
                headers=headers,
            )
            checkpoint_retention_crlf = client.post(
                "/api/v1/blockchain/maintenance/cleanup?checkpoint_retention_seconds=%0D%0A",
                headers=headers,
            )
            max_network_events_crlf = client.post(
                "/api/v1/blockchain/maintenance/cleanup?max_network_events=%0D%0A",
                headers=headers,
            )

        self.assertEqual(event_retention_crlf.status_code, 422)
        self.assertEqual(checkpoint_retention_crlf.status_code, 422)
        self.assertEqual(max_network_events_crlf.status_code, 422)
    ...
    def test_cleanup_endpoint_rejects_negative_query_parameters(self) -> None:
        headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
        with TestClient(app) as client:
            event_retention_negative = client.post(
                "/api/v1/blockchain/maintenance/cleanup?event_retention_seconds=-1",
                headers=headers,
            )
            checkpoint_retention_negative = client.post(
                "/api/v1/blockchain/maintenance/cleanup?checkpoint_retention_seconds=-1",
                headers=headers,
            )
            max_network_events_negative = client.post(
                "/api/v1/blockchain/maintenance/cleanup?max_network_events=-1",
                headers=headers,
            )

        self.assertEqual(event_retention_negative.status_code, 422)
        self.assertEqual(checkpoint_retention_negative.status_code, 422)
        self.assertEqual(max_network_events_negative.status_code, 422)
    ...
    def test_operation_start_intent_rejects_non_numeric_hashrate(self) -> None:
        _, session_id = self._create_player_session_binding()
        ...
        with TestClient(app) as client:
            response = client.post(
                f"/api/v1/blockchain/operations/intents/start?session_id={session_id}",
                json={
                    "operation_id": "op_invalid_hashrate_type",
                    "base_hashrate_hps": "not_a_number",
                },
            )

        self.assertEqual(response.status_code, 422)

    def test_operation_start_intent_rejects_whitespace_hashrate(self) -> None:
        _, session_id = self._create_player_session_binding()
        ...
        with TestClient(app) as client:
            response = client.post(
                f"/api/v1/blockchain/operations/intents/start?session_id={session_id}",
                json={
                    "operation_id": "op_invalid_hashrate_whitespace",
                    "base_hashrate_hps": "   ",
                },
            )
    ...
    def test_operation_start_intent_rejects_empty_hashrate(self) -> None:
        _, session_id = self._create_player_session_binding()
        ...
        with TestClient(app) as client:
            response = client.post(
                f"/api/v1/blockchain/operations/intents/start?session_id={session_id}",
                json={
                    "operation_id": "op_invalid_hashrate_empty",
                    "base_hashrate_hps": "",
                },
            )
    ...
    def test_operation_stop_intent_rejects_missing_session_transport(self) -> None:
        with TestClient(app) as client:
            response = client.post(
                ...
                json={"operation_id": "op_missing_transport"},
            )
    ...
    def test_operation_intents_reject_empty_session_id_query_transport(self) -> None:
        with TestClient(app) as client:
            start_response = client.post(
                ...
                json={
                    "operation_id": "op_empty_session_start",
                    "base_hashrate_hps": "20",
                },
            )
            stop_response = client.post(
                "/api/v1/blockchain/operations/intents/stop?session_id=",
                json={"operation_id": "op_empty_session_stop"},
            )
    ...
    def test_operation_intents_reject_invalid_session_id_query_transport(self) -> None:
        with TestClient(app) as client:
            start_response = client.post(
                "/api/v1/blockchain/operations/intents/start?session_id=not-a-session",
                json={
                    "operation_id": "op_invalid_query_session_start",
                    "base_hashrate_hps": "20",
                },
            )
            stop_response = client.post(
                "/api/v1/blockchain/operations/intents/stop?session_id=not-a-session",
                json={"operation_id": "op_invalid_query_session_stop"},
            )
    ...
    def test_operation_intents_reject_whitespace_session_id_query_transport(self) -> None:
        with TestClient(app) as client:
            start_response = client.post(
                "/api/v1/blockchain/operations/intents/start?session_id=%20%20%20",
                json={
                    "operation_id": "op_whitespace_query_session_start",
                    "base_hashrate_hps": "20",
                },
            )
            stop_response = client.post(
                "/api/v1/blockchain/operations/intents/stop?session_id=%20%20%20",
                json={"operation_id": "op_whitespace_query_session_stop"},
            )
    ...
    def test_operation_intents_reject_empty_session_header_transport(self) -> None:
        header_name = settings.operation_intent_session_header
        ...
        with TestClient(app) as client:
            start_response = client.post(
                "/api/v1/blockchain/operations/intents/start",
                json={
                    "operation_id": "op_empty_header_start",
                    "base_hashrate_hps": "20",
                },
                headers={header_name: ""},
            )
            stop_response = client.post(
                "/api/v1/blockchain/operations/intents/stop",
                json={"operation_id": "op_empty_header_stop"},
                headers={header_name: ""},
            )
    ...
    def test_operation_intents_reject_invalid_session_header_transport(self) -> None:
        header_name = settings.operation_intent_session_header
        ...
        with TestClient(app) as client:
            start_response = client.post(
                "/api/v1/blockchain/operations/intents/start",
                json={
                    "operation_id": "op_invalid_header_start",
                    "base_hashrate_hps": "20",
                },
            ...
            stop_response = client.post(
                "/api/v1/blockchain/operations/intents/stop",
                json={"operation_id": "op_invalid_header_stop"},
                headers={header_name: "not-a-session"},
            )
    ...
    def test_operation_intents_whitespace_session_transport_increments_query_and_header_counters(self) -> None:
        header_name = settings.operation_intent_session_header

        with TestClient(app) as client:
            whitespace_query = client.post(
                "/api/v1/blockchain/operations/intents/start?session_id=%20%20%20",
                json={
                    "operation_id": "op_whitespace_counter_query",
                    "base_hashrate_hps": "20",
                },
            )
            whitespace_header = client.post(
                "/api/v1/blockchain/operations/intents/stop",
                json={"operation_id": "op_whitespace_counter_header"},
                headers={header_name: "   "},
            )
            metrics_headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
            metrics_json = client.get("/api/v1/blockchain/maintenance/metrics", headers=metrics_headers)
        ...
        self.assertEqual(whitespace_query.status_code, 401)
        self.assertEqual(whitespace_header.status_code, 401)
        self.assertEqual(metrics_json.status_code, 200)
        self.assertEqual(metrics_plaintext.status_code, 200)
    ...
    def test_operation_intents_reject_newline_whitespace_query_session_transport(self) -> None:
        with TestClient(app) as client:
            start_response = client.post(
                "/api/v1/blockchain/operations/intents/start?session_id=%0A",
                json={
                    "operation_id": "op_newline_query_session_start",
                    "base_hashrate_hps": "20",
                },
            )
            stop_response = client.post(
                "/api/v1/blockchain/operations/intents/stop?session_id=%0A",
                json={"operation_id": "op_newline_query_session_stop"},
            )
    ...
    def test_operation_intents_newline_whitespace_query_transport_increments_query_counter(self) -> None:
        with TestClient(app) as client:
            response = client.post(
                "/api/v1/blockchain/operations/intents/start?session_id=%0A",
                json={
                    "operation_id": "op_newline_query_counter",
                    "base_hashrate_hps": "20",
                },
            )
            metrics_headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
    ...
    def test_operation_intents_reject_carriage_return_whitespace_query_session_transport(self) -> None:
        with TestClient(app) as client:
            start_response = client.post(
                "/api/v1/blockchain/operations/intents/start?session_id=%0D",
                json={
                    "operation_id": "op_carriage_query_session_start",
                    "base_hashrate_hps": "20",
                },
            )
            stop_response = client.post(
                "/api/v1/blockchain/operations/intents/stop?session_id=%0D",
                json={"operation_id": "op_carriage_query_session_stop"},
            )
    ...
    def test_operation_intents_carriage_return_query_transport_increments_query_counter(self) -> None:
        with TestClient(app) as client:
            response = client.post(
                "/api/v1/blockchain/operations/intents/start?session_id=%0D",
                json={
                    "operation_id": "op_carriage_query_counter",
                    "base_hashrate_hps": "20",
                },
            )
            metrics_headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
    ...
    def test_operation_intents_reject_crlf_whitespace_query_session_transport(self) -> None:
        with TestClient(app) as client:
            start_response = client.post(
                "/api/v1/blockchain/operations/intents/start?session_id=%0D%0A",
                json={
                    "operation_id": "op_crlf_query_session_start",
                    "base_hashrate_hps": "20",
                },
            )
            stop_response = client.post(
                "/api/v1/blockchain/operations/intents/stop?session_id=%0D%0A",
                json={"operation_id": "op_crlf_query_session_stop"},
            )
    ...
    def test_operation_intents_strict_mode_accepts_dual_transport_and_tracks_dual_match(self) -> None:
        _, session_id = self._create_player_session_binding()
        header_name = settings.operation_intent_session_header
        original_strict_mode = settings.operation_intent_require_header_binding
        settings.operation_intent_require_header_binding = True
        ...
        try:
            with TestClient(app) as client:
                start_response = client.post(
                    f"/api/v1/blockchain/operations/intents/start?session_id={session_id}",
                    json={
                        "operation_id": "op_strict_dual_match",
                        "base_hashrate_hps": "20",
                    },
                    headers={header_name: session_id},
                )
                stop_response = client.post(
                    f"/api/v1/blockchain/operations/intents/stop?session_id={session_id}",
                    json={"operation_id": "op_strict_dual_match"},
                    headers={header_name: session_id},
                )
                metrics_headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
                metrics_response = client.get("/api/v1/blockchain/maintenance/metrics", headers=metrics_headers)
    ...
    def test_operation_intents_strict_mode_query_rejections_increment_counter_exactly(self) -> None:
        ...
        try:
            ...
            self.assertIn('gmn_operation_intent_transport_requests_total{mode="query_rejected_strict"} 2', metrics_plaintext.text)
        finally:
            settings.operation_intent_require_header_binding = original_strict_mode
    ...
    def test_operation_intents_carriage_return_header_transport_increments_header_counter(self) -> None:
        header_name = settings.operation_intent_session_header
        ...
        with TestClient(app) as client:
            response = client.post(
                "/api/v1/blockchain/operations/intents/start",
                json={
                    "operation_id": "op_carriage_header_counter",
                    "base_hashrate_hps": "20",
                },
                headers={header_name: "\r"},
            )
            metrics_headers = {settings.maintenance_auth_header: settings.maintenance_auth_token}
            metrics_json = client.get("/api/v1/blockchain/maintenance/metrics", headers=metrics_headers)
            metrics_plaintext = client.get("/api/v1/blockchain/maintenance/metrics/plaintext", headers=metrics_headers)
    ...
    def test_operation_intents_crlf_header_transport_increments_header_counter(self) -> None:
        header_name = settings.operation_intent_session_header
        ...
        with TestClient(app) as client:
            response = client.post(
                "/api/v1/blockchain/operations/intents/start",
                json={
                    "operation_id": "op_crlf_header_counter",
                    "base_hashrate_hps": "20",
                },
                headers={header_name: "\r\n"},
            )
    ...
    def test_maintenance_endpoints_reject_empty_auth_token(self) -> None:
        with TestClient(app) as client:
            ...
            unauthorized_cleanup = client.post("/api/v1/blockchain/maintenance/cleanup", headers=empty_headers)
            unauthorized_json = client.get("/api/v1/blockchain/maintenance/metrics", headers=empty_headers)
            unauthorized_plaintext = client.get(
                "/api/v1/blockchain/maintenance/metrics/plaintext",
    ...
    def test_checkpoint_player_rewards_upsert_rejects_missing_reconnect_cursor(self) -> None:
        player_id, session_id = self._create_player_session_binding()
    ...
    def test_checkpoint_upsert_rejects_non_integer_reconnect_cursor(self) -> None:
        player_id, session_id = self._create_player_session_binding()

        with TestClient(app) as client:
            global_response = client.put(
                f"/api/v1/blockchain/checkpoints/global?player_id={player_id}&session_id={session_id}",
                json={"reconnect_cursor": "not_an_int"},
            )
    ...
    def test_checkpoint_upsert_rejects_empty_reconnect_cursor(self) -> None:
        player_id, session_id = self._create_player_session_binding()

        with TestClient(app) as client:
            global_response = client.put(
                f"/api/v1/blockchain/checkpoints/global?player_id={player_id}&session_id={session_id}",
                json={"reconnect_cursor": ""},
            )
            rewards_response = client.put(
                f"/api/v1/blockchain/checkpoints/player_rewards?player_id={player_id}&session_id={session_id}",
                json={"reconnect_cursor": ""},
    ...
    def test_checkpoint_upsert_rejects_whitespace_reconnect_cursor(self) -> None:
        player_id, session_id = self._create_player_session_binding()
    ...
    def test_checkpoint_upsert_rejects_fractional_reconnect_cursor(self) -> None:
        player_id, session_id = self._create_player_session_binding()

        with TestClient(app) as client:
            global_response = client.put(
                f"/api/v1/blockchain/checkpoints/global?player_id={player_id}&session_id={session_id}",
                json={"reconnect_cursor": 1.5},
            )
            rewards_response = client.put(
                f"/api/v1/blockchain/checkpoints/player_rewards?player_id={player_id}&session_id={session_id}",
                json={"reconnect_cursor": 1.5},
    ...
    def test_checkpoint_player_rewards_endpoints_require_player_and_session_query_parameters(self) -> None:
        with TestClient(app) as client:
            ...
            get_missing_session = client.get("/api/v1/blockchain/checkpoints/player_rewards?player_id=player_only")
            put_missing_player = client.put(
                "/api/v1/blockchain/checkpoints/player_rewards?session_id=session_only",
                json={"reconnect_cursor": 3},
            )
            put_missing_session = client.put(
                "/api/v1/blockchain/checkpoints/player_rewards?player_id=player_only",
                json={"reconnect_cursor": 3},
            )
    ...
    def test_checkpoint_player_rewards_endpoints_reject_empty_player_or_session_values(self) -> None:
        with TestClient(app) as client:
            get_empty_player = client.get("/api/v1/blockchain/checkpoints/player_rewards?player_id=&session_id=session")
            get_empty_session = client.get("/api/v1/blockchain/checkpoints/player_rewards?player_id=player&session_id=")
            put_empty_player = client.put(
                "/api/v1/blockchain/checkpoints/player_rewards?player_id=&session_id=session",
                json={"reconnect_cursor": 3},
            )
            put_empty_session = client.put(
                "/api/v1/blockchain/checkpoints/player_rewards?player_id=player&session_id=",
                json={"reconnect_cursor": 3},
            )
    ...
    def test_checkpoint_endpoints_reject_invalid_non_empty_player_or_session_values(self) -> None:
        with TestClient(app) as client:
            global_get = client.get("/api/v1/blockchain/checkpoints/global?player_id=invalid&session_id=invalid")
            global_put = client.put(
                "/api/v1/blockchain/checkpoints/global?player_id=invalid&session_id=invalid",
                json={"reconnect_cursor": 3},
            )
            rewards_get = client.get(
                "/api/v1/blockchain/checkpoints/player_rewards?player_id=invalid&session_id=invalid"
            )
